...
type OrangeBookClient = {
  ...
  searchProducts: (...args: unknown[]) => Promise<Record<string, unknown>>;
};
...
async function getOrangeBookClient(): Promise<OrangeBookClient | null> {
  ...
  try {
    ...
    fdaOrangeBookClient = mod.fdaOrangeBookClient as OrangeBookClient;
  } catch {
...
export const drugSafetyResearchTool: DataSourceTool = {
  ...
  handler: async (input: Record<string, unknown>, _cache: ToolCache): Promise<ToolResult> => {
    ...
    const seriousCount = aeResult.data.results.filter(
      (r) => (r as unknown as Record<string, unknown>).serious === 1,
    ).length;
    ...
    const topReactions = countResult.data.results.slice(0, 5).map((r) => ({
      term: String((r as unknown as Record<string, unknown>).term ?? "Unknown"),
      count: (r as unknown as Record<string, unknown>).count as number ?? 0,
    }));

    const label = labelResult.data.results[0] as unknown as Record<string, unknown> | undefined;
