...
export interface SlideHTML {
  ...
  /** Structured content for editor persistence (populated at generation time) */
...
export interface SlideGeneratorInput {
  ...
  /** Binding composition contract — validated post-generation */
  compositionSpec?: SlideCompositionSpec;
}
...
// ─── Slide Structure (for editor persistence) ────────────────

/** Structured content emitted alongside HTML at generation time.
 *  This is persisted to SlideVersion.content and consumed by the editor UI. */
...
// ─── Slide Composition Spec (quality enforcement) ────────────

/** Binding composition contract per slide — generated by planner, validated post-generation */
export interface SlideCompositionSpec {
  /** CSS class names this slide MUST use (from design system vocabulary) */
  requiredComponentClasses: string[];
  /** Minimum distinct animation types (e.g., 2 = at least anim-blur AND anim-spring) */
  minAnimationTypes: number;
  /** Must include interactive components from the design system */
  interactiveRequirement: "none" | "one" | "multiple";
  /** Target visual density */
  densityTarget: "sparse" | "standard" | "rich";
  /** Assigned background variant (not suggested — assigned) */
  backgroundVariant: string;
  /** Chart requirements when data is available */
  chartRequirement: "none" | "one" | "multiple" | "mixed-types";
}

/** Result of validating a slide against its composition spec */
...
export interface CompositionViolation {
  ...
  detail: string;
  /** Concrete fix suggestion passed to remediator */
  suggestion: string;
}

/** Deck-level quality dimensions for design review */
export interface DeckCompositionReview {
  componentVocabulary: number;    // distinct classes used / 30 target
  animationDiversity: number;     // distinct animation types / 6 target
  chartTypeDiversity: number;     // distinct chart types / 4 target
  interactiveRichness: number;    // % of content slides with interactive components
  backgroundAlternation: number;  // penalize adjacent same backgrounds
  visualRhythm: number;           // density variation across slides
  overallDesignScore: number;     // weighted composite
}
...
export interface QualityMetrics {
  ...
  // v2: Enhanced visual design metrics
  componentVocabulary?: MetricScore;
  interactiveRichness?: MetricScore;
  compositionCompliance?: MetricScore;
}
...
export interface RemediationChartFragment {
  type: string;
  markup: string;
  slotName?: string;
}
...
export interface RemediationInput {
  ...
  exemplarHtml: string;
  chartFragments: RemediationChartFragment[];
  /** Composition violations from per-slide validation — includes concrete fix suggestions */
  compositionViolations?: CompositionViolation[];
}
...
export type ComponentSlotValue = Record<string, string | undefined>;
export type ContentSlotValue = string | StatData | ListItem[] | ComponentSlotValue;

export interface ContentGeneratorOutput {
  slots: Record<string, ContentSlotValue>;
